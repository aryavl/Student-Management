...
const AddSubjectModal: React.FC<SingleStreamContentProps> = ({ id }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [errorr, setError] = useState("");
  const router = useRouter();
  const { data, error } = useSWR(
    `http://localhost:3000/api/admin/stream/subjects/register?id=${id}`,
    async (url) => {
  ...
  const handleSubmit = async (e: HandleSubmitType) => {
    ...
    const stream = e.target[0].value;
    const streamId = e.target[1].value;
    const subject = e.target[2].value;
    
    console.log(stream,streamId,subject);
    
    const res = await fetch("/api/admin/stream/subjects/register", {
      method: "POST",
      headers: {
        ...
        <div className="fixed inset-0 bg-gray-700 bg-opacity-75 flex items-center justify-center">
          <div className="bg-white p-8 rounded shadow-md w-1/2">
            ...
            <div className="mb-4">
                ...
                <input className="text-gray-900" type="text" name="" id="" value={streams.streamName} disabled/>
              </div>
            <div className="mb-4">
                <label
                  htmlFor="username"
                ...
                <input className="text-gray-900" type="hidden" name="" id="" value={streams._id} hidden/>
              </div>
             <div className="mb-4">
               <label
                 htmlFor="username"
