...
const StaffLogin = () => {
  ...
  //   `/api/teacher/password?email=${staffEmail}`,
  //   async (url) => {
  ...
  console.log(data);
        ...
        if(res.status === 200){
          ...
          if(data?.verifiedTeacher){
            router.push(`/staff/login/form?email=${email}`)

          }
          if(data?.updatedTeacher){

            router.push(`/staff/login/otp?email=${email}`)
          }
        }
      } catch (error:any) {
        ...
        <div className="pt-8 pr-8 pb-8 pl-8 lg:col-span-7">
          ...
          <p className="text-red-600 text=[16px] mb-4">{error && error}</p>
            ...
            <button type='submit' className="hover:bg-gray-600 rounded-md text-xl pt-3 pr-3 pb-3 pl-3 bg-gray-800
                font-semibold text-white w-full text-center mt-5">Submit</button>
